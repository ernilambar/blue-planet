...
if ( ! function_exists( 'blue_planet_add_secondary_slider_function' ) ) :
	...
	function blue_planet_add_secondary_slider_function() {
		...
		<?php if ( $secondary_slider_query->have_posts() ) : ?>
			<div class="secondary-slider-wrapper theme-default">
				<div class="ribbon"></div>
						...
						<div id="bp-secondary-slider" class="nivoSlider">
							...
							<a href="<?php echo esc_url( $slide['permalink'] ); ?>">
								...
								if ( 1 === $bp_options['slider_caption_2'] ) {
									$img_attrs['title'] = '#htmlcaption-' . $slide['ID'];
								}
								?>
								<img <?php blue_planet_render_attr( $img_attrs ); ?>>
							</a>
						<?php endforeach; ?>
						</div>
					...
					<?php if ( ! empty( $slide_data ) ) : ?>
						...
						<?php foreach ( $slide_data as $slide_item ) : ?>
							<div id="<?php echo esc_attr( 'htmlcaption-' . $slide_item['ID'] ); ?>" class="nivo-html-caption">
								<h4><?php echo $slide_item['title']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></h4>
									<?php echo $slide_item['excerpt']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</div><!-- .nivo-html-caption -->
							<?php endforeach; ?>

					<?php endif; ?>

			</div><!-- .secondary-slider-wrapper -->
	...
	function blue_planet_footer_powered_by() {
		...
		<div class="footer-powered-by" style="<?php echo esc_attr( $extra_style ); ?>">
			<a href="<?php echo esc_url( __( 'https://wordpress.org/', 'blue-planet' ) ); ?>">
				...
				printf( esc_html__( 'Powered by %s', 'blue-planet' ), 'WordPress' );
				?>
			</a>
			<span class="sep"> | </span>
			<?php
			/* translators: 1: Theme name, 2: Theme author. */
			printf( esc_html__( '%1$s by %2$s', 'blue-planet' ), 'Blue Planet', '<a href="https://www.nilambar.net" rel="designer">Nilambar</a>' );
			?>
		</div>
		<?php
	}
			...
			<div class="main-slider-wrapper">
				...
				<div id="bp-main-slider" class="nivoSlider">
					<?php foreach ( $slides as $slide ) : ?>
						...
						<?php
						echo '<img src=" ' . esc_url( $slide['image'] ) . '" title="' . esc_attr( $slide['caption'] ) . '" />';  // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
						?>
	...
	function blue_planet_header_content_stuff() {
		...
		if ( ! empty( $header_image ) ) :
			...
			<div class="header-image-wrapper">
				...
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home"><img id="bs-header-image" src="<?php header_image(); ?>" width="<?php echo esc_attr( get_custom_header()->width ); ?>" height="<?php echo esc_attr( get_custom_header()->height ); ?>" alt="" /></a>
