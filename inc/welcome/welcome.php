...
add_action(
	'wp_welcome_init',
	function() {
		$obj = new Welcome( 'theme', 'blue-planet' );

		$obj->set_page(
			array(
				'menu_title'  => esc_html__( 'Blue Planet', 'blue-planet' ),
				'page_title'  => esc_html__( 'Blue Planet', 'blue-planet' ),
				/* translators: %s: Version */
				'page_subtitle' => sprintf( esc_html__( 'Version: %s', 'blue-planet' ), BLUE_PLANET_VERSION ),
				'menu_slug'   => 'blue-planet-welcome',
		...
		$obj->set_quick_links(
			array(
				array(
					'text' => 'Theme Details',
					'url'  => 'https://www.nilambar.net/2014/03/blue-planet-wordpress-theme-free.html',
					'type' => 'primary',
				),
				array(
					'text' => 'Get Support',
					'url'  => 'https://wordpress.org/support/theme/blue-planet/#new-post',
					'type' => 'secondary',
				),
				array(
					'text' => 'Leave a Review',
					'url'  => 'https://wordpress.org/support/theme/blue-planet/reviews/#new-post',
					'type' => 'secondary',
				),
			)
		);
			...
			array(
				...
				'items' => array(
					...
					array(
						...
						'button_url'  => 'https://wordpress.org/support/theme/blue-planet/#new-post',
		...
		$obj->set_sidebar(
			array(
				'render_callback' => 'blue_planet_render_welcome_page_sidebar',
			)
		);
 ...
 * @since 1.0.0
 */
function blue_planet_get_blog_posts_ajax_callback() {
	$output = array();

	$posts = blue_planet_get_blog_feed_items();
...
add_action( 'wp_ajax_nopriv_blue_planet_nsbl_get_posts', 'blue_planet_get_blog_posts_ajax_callback' );
add_action( 'wp_ajax_blue_planet_nsbl_get_posts', 'blue_planet_get_blog_posts_ajax_callback' );
 ...
 */
function blue_planet_render_welcome_page_sidebar( $object ) {
	$object->render_sidebar_box(
		array(
			...
			'button_url'   => 'https://wordpress.org/support/theme/blue-planet/reviews/#new-post',
			'button_class' => 'button',
		),
		$object
	);
 ...
 */
function blue_planet_load_welcome_assets( $hook ) {
	if ( 'appearance_page_blue-planet-welcome' !== $hook ) {
		return;
	}

	$min = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';

	wp_enqueue_script( 'blue-planet-blog-posts', get_template_directory_uri() . '/js/blog-posts' . $min . '.js', array( 'jquery' ), BLUE_PLANET_VERSION, true );
}

add_action( 'admin_enqueue_scripts', 'blue_planet_load_welcome_assets' );
